 ...
 * @param {String=} options.skipRequests - Optional flag to skip certain requests in the final report.
...
PostmanHTMLExtraReporter = function (newman, options, collectionRunOptions) {
    ...
    // Helper for checking whether request name is in skip list
    handlebars.registerHelper('containsRequest', function (item) {
        return _.isString(options.skipRequests) && options.skipRequests.split(',').includes(item)
    });
